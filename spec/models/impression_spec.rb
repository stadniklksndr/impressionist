...
RSpec.describe Impression do
  ...
  describe "deletes impressions on deletion of impressionable" do
    let(:article) { Article.create }
    let!(:impression) { article.impressions.create }

    before do
      article.destroy
    end

    it { expect(article).to be_destroyed }
    it { expect(described_class).not_to exist(impression.id) }
  end
end
